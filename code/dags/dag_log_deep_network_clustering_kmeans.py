...
def build_autoencoder(input_dim, latent_dim):
    with tracer.start_as_current_span("build_autoencoder") as span:
        span.set_attribute("function", "build_autoencoder")
...
def train_autoencoder_kmeans_pipeline():
    with tracer.start_as_current_span("train_autoencoder_kmeans_pipeline") as span:
        span.set_attribute("function", "train_autoencoder_kmeans_pipeline")
        logger.info("🚀 Starting autoencoder + KMeans pipeline")
        logger.info(f"Tensor flow version: {tf.__version__}")
        ...
        logger.info("Converting logs to TF-IDF vectors")
        vectorizer = TfidfVectorizer(max_features=MAX_FEATURES)
        X = vectorizer.fit_transform(sequences).toarray()

        logger.info("Building and training autoencoder")
        ...
        logger.info("🔎 Extracting latent features and applying KMeans")
        ...
        logger.info(f"☁️ Model uploaded to s3://{S3_BUCKET}/{S3_MODEL_KEY}")
